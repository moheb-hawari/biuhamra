...
class Settings extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        Assets::add_css(assets_path().'globals/plugins/select2/select2.css');
        Assets::add_css(assets_path().'globals/plugins/select2/select2-bootstrap.css');
        ...
        Assets::add_js(assets_path().'globals/plugins/select2/select2.min.js');
        Assets::add_js(assets_path().'globals/js/states_cities.js');
    ...
    public function index($filter = 'all', $offset = 0)
    {
        ...
        } elseif (preg_match('{role_date-([0-9]*)}', $filter, $matches)) {
            $filterType = 'role_date';
            $roleId = (int) isset($matches[1][0]) && $matches[1][0] ==1 ? 2 : 4;
        ...
        switch ($filterType) {
            ...
            case 'role_date':
                ...
                // usable with databases other than MySQL
                $where['users.role_id'] = $roleId;
                $where['MONTH('.$this->db->dbprefix.'users.created_on)'] = $month;
                break;
            case 'all':
                // Nothing to do
                break;
    ...
    public function profile()
    {
        ...
        if (isset($_POST['save'])) {
            ...
            if ($this->saveUser('update', $user_id, $meta_fields)) {
                ...
                Template::redirect('/admin/settings/users/profile');
            }
        ...
        Template::set('user', $user);
        Template::set('languages', unserialize($this->settings_lib->item('site.languages')));
        Template::set('toolbar_title', lang('us_edit_profile'));
