...
if (! empty($meta_fields)) :
    ...
    foreach ($meta_fields as $field) :
        ...
        elseif ($field['form_detail']['type'] == 'state') : ?>

            <div class="inputer floating-label control-group<?php echo form_error($field['name']) ? ' error' : ''; ?>">
                <?php echo form_label($field['label']. lang('bf_form_label_required'), $field['name'], array('class' => 'control-label')); ?>
                <div class='input-wrapper controls'>
                    <input class="form-control form-filter  select2" id='states' type='text' required='required' name='state' value="<?php echo set_value('state', isset($user->{$field['name']}) ? $user->{$field['name']} : 'TX'); ?>" />
                    <span class='help-inline'><?php echo form_error($field['name']); ?></span>
                </div>
            </div>

        <?php elseif ($field['form_detail']['type'] == 'city') : ?>

            <div class="inputer floating-label control-group<?php echo form_error($field['name']) ? ' error' : ''; ?>">
                <?php echo form_label($field['label'] . lang('bf_form_label_required'), $field['name'], array('class' => 'control-label')); ?>
                <div class='input-wrapper controls'>
                    <input class="form-control form-filter select2" id='cities' type='text' required='required' name='city' value="<?php echo set_value($field['name'], isset($user->{$field['name']}) ? $user->{$field['name']} : 6436 ); ?>" />
                    <span class='help-inline'><?php echo form_error($field['name']); ?></span>
                </div>
            </div>
...
<div class="control-group<?php echo form_error($field['name']) ? ' error' : ''; ?>">
    ...
    <div class="controls">
        ...
        echo state_select(
            ...
            $defaultCountry,
            $field['name'],
            'selectpicker span6 chzn-select'
        );
        ?>
    </div>
</div>
