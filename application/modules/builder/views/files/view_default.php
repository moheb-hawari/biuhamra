    ...
    switch ($field_type) {
        ...
        case 'select':
            ...
            $viewFields .= "
                ...
                echo form_dropdown(array('name' => '{$form_name}'" . ($required_attribute ? ",'class'=>'selectpicker','required' => 'required'" : "") . "), \$options, set_value('{$form_name}', isset(\${$module_name_lower}->{$field_name}) ? \${$module_name_lower}->{$field_name} : ''), lang('{$module_name_lower}_field_{$field_name}'){$required},'','','col-md-6');
            ?>";
            break;
        case 'checkbox':
            $viewFields .= PHP_EOL . "
        ...
        default:
            ...
            $viewFields .= PHP_EOL . "
            <div class=\"inputer floating-label control-group<?php echo form_error('{$field_name}') ? ' error' : ''; ?>\">
                <?php echo form_label(lang('{$module_name_lower}_field_{$field_name}'){$required}, '{$form_name}', array('class' => 'control-label')); ?>
                <div class='input-wrapper controls'>
                    <input class='form-control' id='{$form_name}' type='{$type}' " . ($required_attribute ? "required='required' " : "") . "name='{$form_name}' {$maxlength} value=\"<?php echo set_value('{$form_name}', isset(\${$module_name_lower}->{$field_name}) ? \${$module_name_lower}->{$field_name} : ''); ?>\" />
                        <label for='{$form_name}'><?php echo lang('{$module_name_lower}_field_{$field_name}'); ?></label>
                    <span class='help-inline'><?php echo form_error('{$field_name}'); ?></span>
                </div>
            </div>";
            break;
...
<div class='admin-box panel'>
    <div class='panel-heading'>
        <div class='panel-title'><h3>{$module_name}</h3></div>
    </div>
